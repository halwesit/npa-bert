...
class BaseModel:
    ...
    def _get_loss(self):
        ...
        if self.hparams.loss == "cross_entropy_loss":
            ...
            raise ValueError(
                "this loss not defined {0}".format(self.hparams.loss))
        return data_loss
    ...
    def eval(self, eval_batch_data):
        ...
        """
        eval_input, eval_label = self._get_input_label_from_iter(
            eval_batch_data)
    ...
    def run_user(self, news_filename, behaviors_file):
        ...
        for batch_data_input in tqdm(
            self.test_iterator.load_user_from_file(
                news_filename, behaviors_file)
        ):
    ...
    def run_slow_eval(self, news_filename, behaviors_file):
        ...
        for batch_data_input in tqdm(
            self.test_iterator.load_data_from_file(
                news_filename, behaviors_file)
        ):
            step_pred, step_labels, step_imp_index = self.eval(
                batch_data_input)

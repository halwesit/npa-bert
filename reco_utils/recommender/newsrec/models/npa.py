...
class NPAModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder, user_embedding_layer):
        ...
        repeat_uids = layers.Concatenate(
            axis=-2)([nuser_id] * hparams.his_size)
        his_title_uid = layers.Concatenate(
            axis=-1)([his_input_title, repeat_uids])

        click_title_presents = layers.TimeDistributed(
            titleencoder)(his_title_uid)
    ...
    def _build_newsencoder(self, embedding_layer, user_embedding_layer):
        ...
        model = keras.Model(sequence_title_uindex,
    ...
    def _build_npa(self):
        ...
        titleencoder = self._build_newsencoder(
            embedding_layer, user_embedding_layer)
        userencoder = self._build_userencoder(
            titleencoder, user_embedding_layer)
        ...
        model = keras.Model(
            [user_indexes, his_input_title, pred_input_title], preds)
        scorer = keras.Model(
